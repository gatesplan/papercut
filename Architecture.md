...
- [확정] Extractor는 MinerU 3.x 로컬 API 서버(mineru-api)를 서브프로세스로 관리한다. 채택 이유: in-process 방식은 mineru 내부 전역 모델 캐시 때문에 VRAM 확실 해제가 불가능하며, 프로세스 종료만이 반납을 보장한다. [근거, .meta/260710-미네루3.4API조사.md]
- [확정] 로드는 lazy(첫 extract 호출 시 서브프로세스 기동), unload()는 서브프로세스 종료로 VRAM/RAM을 OS 수준에서 반납한다. 디스크 가중치는 unload와 무관하게 유지된다.
- 가중치 다운로드는 서버측 첫 파싱 시 자동 수행된다 (생성자 시점 아님, 서브프로세스 방식 채택에 따른 변경). 서버 배포 시 mineru-models-download 사전 다운로드 권장.
...
- 다중 객체 동시 최초 다운로드 충돌은 HuggingFace 다운로더의 파일 락에 의존한다. 사전 다운로드(mineru-models-download)로 원천 회피 가능.
